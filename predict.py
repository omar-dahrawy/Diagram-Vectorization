...
path = 'Data/Cropped Shapes/'

for filename in glob.glob(path + '*.jpg'):
    im = Image.open(filename)
    image_list.append(im)
    
images_names = [f for f in os.listdir(path) if os.path.splitext(f)[-1] == '.jpg']
...
label = {0: "Circle", 1: "Line", 2: "Square", 3: "Triangle"}
...
def predict_one(file_name):
    ...
    with open("Data/categories.txt", "a") as text_file:
...
def main():
    ...
    parser.add_argument('--testdata', help='Classify images on test data', action='store_true')
    parser.add_argument('--validationdata', help='Classify images on test data', action='store_true')
